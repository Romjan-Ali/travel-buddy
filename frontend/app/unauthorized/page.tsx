...
const reasonMessages: Record<string, { title: string; description: string; icon: React.ReactNode }> = {
  'admin-access-required': {
    ...
    description: 'This page is restricted to administrators only. Please contact your system administrator if you believe you should have access.',
  ...
  'verification-required': {
    ...
    description: 'Your account needs to be verified before accessing this feature. Please check your email for verification instructions.',
    icon: <Shield className="h-6 w-6" />
  }
}
...
export default function UnauthorizedPage() {
  ...
  const [reason, setReason] = useState('admin-access-required')

  useEffect(() => {
    const reasonParam = searchParams.get('reason')
    if (reasonParam && reasonMessages[reasonParam]) {
        ...
        <CardFooter className="flex flex-col gap-3">
          ...
          <div className="text-center w-full pt-4 border-t">
            <p className="text-xs text-muted-foreground">
              Error Code: <code className="bg-muted px-1.5 py-0.5 rounded text-xs">ACCESS_DENIED_{reason.toUpperCase().replace('-', '_')}</code>
            </p>
