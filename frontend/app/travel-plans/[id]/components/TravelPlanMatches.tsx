...
export function TravelPlanMatches({ travelPlan, travelPlanId }: TravelPlanMatchesProps) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent>
        {travelPlan.matches && travelPlan.matches.length > 0 ? (
          <div className="space-y-4">
            {travelPlan.matches.slice(0, 5).map((match: any) => (
              <div key={match.id} className="flex items-center justify-between p-4 border rounded-lg">
                <div className="flex items-center gap-3">
                  <Avatar>
                    <AvatarImage src={match.initiator?.profile?.profileImage} />
                    <AvatarFallback>
                      {match.initiator?.profile?.fullName?.charAt(0).toUpperCase() || 'T'}
                    </AvatarFallback>
                  </Avatar>
                  <div>
                    ...
                    <Badge variant={
                      ...
                      match.status === 'REJECTED' ? 'destructive' : 'outline'
                    }>
                      {match.status}
                    </Badge>
                  </div>
          ...
          <div className="text-center py-8">
            ...
            <h3 className="font-semibold text-lg mb-2">No match requests yet</h3>
