                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <div className="flex items-center gap-1">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`h-4 w-4 ${i < Math.floor(averageRating) ? 'text-yellow-500 fill-yellow-500' : 'text-gray-300'}`}
                        />
                      ))}
                    </div>
            ...
            {travelPlan.user?.profile?.travelInterests && travelPlan.user?.profile.travelInterests.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {travelPlan.user.profile.travelInterests.map((interest: string, index: number) => (
