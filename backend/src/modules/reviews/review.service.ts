...
async getUserReviews(userId: string, type: 'given' | 'received' = 'received', page: number = 1, limit: number = 10) {
  ...
  async updateReview(reviewId: string, userId: string, updateData: { rating?: number; comment?: string }) {
    ...
    if (review.createdAt < thirtyDaysAgo) {
      throw new AppError(400, 'Reviews can only be edited within 30 days of creation');
    }
