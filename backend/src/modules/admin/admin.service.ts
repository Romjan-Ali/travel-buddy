...
export const adminService = {
  ...
  async updateUserStatus(userId: string, updates: { isActive?: boolean; isVerified?: boolean; role?: string }) {
    const user = await prisma.user.findUnique({
      where: { id: userId },
    });
  ...
  async getSystemAnalytics() {
    ...
    const engagementStats = {
      ...
      plansCreated: await prisma.travelPlan.count({
        where: { createdAt: { gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) } },
      }),
      matchesMade: await prisma.match.count({
        where: { createdAt: { gte: new Date(Date.now() - 30 * 24 * 60 * 60 * 1000) } },
      }),
